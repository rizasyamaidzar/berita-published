@extends('cms.main')
@section('content')
    <div class="p-4 md:m-5">
        <form class="space-y-5" action="{{ route('category.update') }}" method="post">
            @csrf
            <div>
                <input type="hidden" name="id" value="{{ $category->id }}">
                <x-input field="nama" label="Nama" type="text" placeholder="Nama.."
                    value="{{ $category->nama }}"></x-input>
                <x-textarea field="keterangan" label="Keterangan" value="{{ $category->keterangan }}" />
            </div>
            <button type="submit"
    ...
    <div class="m-10 p-5">
        ...
        <table id="search-table" class="dark:bg-darker">
            ...
            <tbody>
                @foreach ($category->berita as $berita)
                    <tr>
